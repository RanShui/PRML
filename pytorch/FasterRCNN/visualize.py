# Routines for visualizing model results and debug.
...
def _draw_rectangle(ctx, corners, color, thickness = 4):
  ...
  ctx.rectangle(outline = color, width = thickness, xy = [(x_min, y_min), (x_max, y_max)])
...
def _class_to_color(class_index):
  return list(ImageColor.colormap.values())[class_index + 2]
...
def show_detections(output_path, show_image, image, scored_boxes_by_class_index, class_index_to_name):
  ...
  if output_path is not None:
    image.save(output_path)
    print("Detection results has been written to '%s'" % output_path)
