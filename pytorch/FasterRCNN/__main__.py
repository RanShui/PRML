# Main module for the PyTorch implementation of Faster R-CNN. 
# Must be run from the root directory, e.g.: python -m pytorch.FasterRCNN --help
# What to do before use
# ---------------------
# 1 see valid_backbones and decide whether it satisfies your need
# 2 went to if __name__ == "__main__" to change default "backbone" and "dataset-dir" if needed
...
from .Image_Processing import voc
...
def train(model):
  ...
  if options.load_from:
    ...
    if options.backbone == "vgg16":  # use pre-trained VGG-16 weights
  ...
  for epoch in range(1, 1 + options.epochs):
    ...
    for sample in progbar:
      ...
      wandb.log(stats.get_wandb_log(iteration=iteration), step=iteration)
...
def generate_prediction(url, base_dir="/home/stu5/ranshui/FasterRCNN/test/results"):
    image_files = [f for f in os.listdir(url) if f.endswith(('.jpg', '.png', '.jpeg', '.bmp'))]

    # Loop through the image files in the folder
    for file_name in image_files:
        # Construct the full path to the image file
        imput_path = os.path.join(url, file_name)
        output_path = os.path.join(base_dir, file_name)
        predict_one(model=model, url = imput_path, show_image=False, output_path=output_path)
...
if  __name__ == "__main__":
  ...
  parser.add_argument("--backbone", metavar = "model", action = "store", default = "resnet50", help = "Backbone model for feature extraction and classification")
  ...
  parser.add_argument("--dataset-dir", metavar = "dir", action = "store", default = "/home/stu5/ranshui/FasterRCNN/VOC", help = "VOC dataset directory")
  ...
  # wandb init
  parser.add_argument('--project', type=str, default='FasterRCNN-ranshui')
  parser.add_argument('--name', type=str)

  options = parser.parse_args()

  # wandb init
  wandb.init(project=options.project, name=options.name)
  ...
  model = FasterRCNNModel(num_classes = voc.Dataset.num_classes, backbone = backbone, 
                          allow_edge_proposals = not options.exclude_edge_proposals).cuda()
  if options.load_from:
  ...
  if options.train:
    ...
    generate_prediction(options.predict_to_file)
  elif not options.dump_anchors:
    print("Nothing. Want to use --train or --predict?")
