# PyTorch implementation of Faster R-CNN training and inference models. 
# For:
# initiation
# RPN mini-batches sampling
# ground truth labels from RPN proposal boxes (RoIs) 
# proposals
...
class FasterRCNNModel(nn.Module):
  ...
  def __init__(self, num_classes, backbone, rpn_minibatch_size = 256, proposal_batch_size = 128, allow_edge_proposals = True):
    ...
    #   Backbone network for feature extraction and pooled feature vector construction.
    # rpn_minibatch_size : int
    #   The number of ground truth anchors sampled for training at each step.
    # proposal_batch_size : int
    #   Number of region proposals to sample at each training step.
    # allow_edge_proposals : bool
    #   Whether to use proposals generated exactly on the edges. 
    super().__init__()
  ...
  def forward(self, image_data, anchor_map = None, anchor_valid_map = None):
    ...
    # Forward inference used for test.

    # Parameters
    # image_data : torch.Tensor
    #   A tensor of batch_size, channels, height, width representing
    #   images normalized on BGR, ImageNet channel-wise mean-centered with the VGG-16 convention.
  ...
  def train_step(self, optimizer, image_data, anchor_map, anchor_valid_map, gt_rpn_map, gt_rpn_object_indices, gt_rpn_background_indices, gt_boxes):
    ...
    #   Optimizer.
    # image_data : (batch_size, channels, height, width)
    # anchor_map : (height, width, num_anchors * 4)
    # anchor_valid_map :(height, width)
    # gt_rpn_map : (batch_size, height, width, num_anchors, 6)
    # gt_rpn_object_indices : List[np.ndarray]
    # gt_rpn_background_indices : List[np.ndarray]
    # gt_boxes : List[List[datasets.training_data.Box]]
    # Return Loss (a dataclass with class and regression losses for both the RPN and detector states).
    self.train()

    # Clear accumulated gradient
    optimizer.zero_grad()

    # For now, only support a batch size of 1
  ...
  def _label_proposals(self, proposals, gt_boxes, min_background_iou_threshold, min_object_iou_threshold):
    ...
    gt_boxes : List[datasets.training_data.Box]
